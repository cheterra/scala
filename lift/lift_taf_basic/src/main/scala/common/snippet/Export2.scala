...
class Export2() {
    ...
    def export(html: NodeSeq) : NodeSeq = {
      println("Export2.export")
      val store = StoreVarReq.getStore
      val toExp = S.param("toExport")
      val expType = S.param("expType")
      println("toExport: " + toExp + ", expType: " + expType)
      expType match { 
        case Full("clip") => copyToClipboard(store.makeTextList(toExp))
        case Full("file") => writeToFile    (store.makeCsvList (toExp))
